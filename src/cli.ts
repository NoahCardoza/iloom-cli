...
program
  ...
  .addOption(
    new Option('--output-format <format>', 'Output format for headless planning runtime (requires --print)')
      .choices(['json', 'stream-json', 'text'])
  )
  .option('--verbose', 'Enable verbose output when supported (requires --print)')
...
program
  ...
  .option('--model <model>', 'Model to use: opus, sonnet, haiku, opus[1m], sonnet[1m], a full Claude model ID, or a Codex model name')
  .addOption(
    new Option('--one-shot <mode>', 'One-shot automation mode')
      .choices(['default', 'noReview', 'bypassPermissions'])
  )
